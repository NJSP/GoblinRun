...
public class VisionCone : MonoBehaviour
{
    ...
    public bool IsPlayerInSight { get; private set; } // Property to check if the player is in sight
    ...
    {
        IsPlayerInSight = false; // Reset the player detection flag
        ...
        for (int i = 0; i < VisionConeResolution; i++)
        {
            ...
            if (Physics.Raycast(transform.position, RaycastDirection, out RaycastHit hit, VisionRange, VisionObstructingLayer))
            {
                ...
                {
                    IsPlayerInSight = true; // Set the flag if the player is detected
                }
            }
            else
            {
